package com.example.brickbrackprojectminorjavafx;
...
public class Controller implements Initializable{
    ...
    Timeline timeline=new Timeline(new KeyFrame(Duration.millis(7),new EventHandler<ActionEvent>(){
        ...
        public void handle(ActionEvent actionEvent) {
            ...
            //check collision of thr ball inside the scene
            check_scene_collision();
            check_brick_collision();
            //check the slider collision
            check_slider_collision();
            //check if there are any bricks available ? if not then game over
            if(all_bricks.isEmpty()){
                //you won, we can proceed to the next level.
                System.exit(10);
            }
        }
    }));
    ...
    public boolean check_collision(Rectangle current_bricks){
        if(circle.getBoundsInParent().intersects(current_bricks.getBoundsInParent())){
            ...
            double leftborder = current_bricks.getLayoutX();
            double rightborder = current_bricks.getLayoutX() + current_bricks.getWidth();
            ...
            if((centre - radius <= rightborder && centre + radius >= leftborder)) {
                rightside = circle.getLayoutX() >= ((current_bricks.getLayoutX() + current_bricks.getWidth()) + circle.getRadius());
                leftside = circle.getLayoutX() <= ((current_bricks.getLayoutX()) - circle.getRadius());
            }
            else {
                upside = circle.getLayoutY() <= ((current_bricks.getLayoutY()) - circle.getRadius());
                bottomside = circle.getLayoutY() >= ((current_bricks.getLayoutY() + current_bricks.getHeight()) - circle.getRadius());
            }
